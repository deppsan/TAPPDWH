...
/**
 * Created by ricar on 17/07/2017.
 */

public class DetallesMisTarjetasUno extends AppCompatActivity implements DetalleServiciosFragment.DetalleServiciosListener
    ...
    ListView lstNavigationCategory;
    ViewPager inferiorViewPager, superiorViewPager;
    ImageView imgTarjeta;
    TextView txtTitle;
    PagerSlidingTabStrip tabLayout;;
    InferiorPageAdapter adapterViewPagerInferior;
    DialogFragment dialog;

    Switch switchDialog;
    View holdView;
    String holdTarjetaNumero;
    TransaccionesAdapter tAdapterHolder;
    int adapterPosition;

    FragmentPagerAdapter adapterViewPagerSuperior;
    int idTarjeta;
    ArrayList<tarjetas.dwh.com.tarjetas.model.Categorias> categoriesImages;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_tarjetas_detalle_1);

        toolbar = (Toolbar) findViewById(R.id.main__toolbar);
        toolbar.setTitle("");
        idTarjeta = getIntent().getIntExtra("idTarjeta",0);
        imgTarjeta = (ImageView) findViewById(R.id.main_backdrop);
        inferiorViewPager  = (ViewPager) findViewById(R.id.materialup_viewpager);
        superiorViewPager  = (ViewPager) findViewById(R.id.materialup_viewpager2);
        tabLayout = (PagerSlidingTabStrip) findViewById(R.id.materialup_tabs);
        ...
        Picasso.with(this).load(R.drawable.tarjeta_andrea).into(imgTarjeta);
        ...
        adapterViewPagerInferior = new InferiorPageAdapter(getSupportFragmentManager(),idTarjeta,this,pages);

        inferiorViewPager.setAdapter(adapterViewPagerInferior);
        superiorViewPager.setAdapter(new SuperiorPageAdapter(getSupportFragmentManager(),idTarjeta));
        tabLayout.setViewPager(inferiorViewPager);
    ...
    public static class InferiorPageAdapter extends FragmentStatePagerAdapter implements PagerSlidingTabStrip.IconTabProvider {
        ...
        private final int[] iconos = {
                R.drawable.bill_andrea
                ,R.drawable.bell_andrea
                ,R.drawable.tag_adrea
                ,R.drawable.cart_andrea
        };
        ...
        public CharSequence getPageTitle(int position) {
            ...
            Drawable image = ResourcesCompat.getDrawable(mcontext.getResources(),iconos[position],null);
    ...
    public void onClickMenu(MenuServiciosDTO menu) {
        switch (menu.getId()){
            ...
            case 2 :
                ...
                break;
            case 3:
                adapterViewPagerInferior.setPages(3,new DetalleServiciosProgramaLealtadFragmentOne());
                break;
            case 4:
                adapterViewPagerInferior.setPages(3, new DetalleServiciosActivarTarjetaFragment());
                break;
    ...
    public class DrawerItemClickListener implements ListView.OnItemClickListener{
        ...
        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
            CategoriasDrawerAdapter  adapter = (CategoriasDrawerAdapter) parent.getAdapter();
            if((adapter.getCount() -1) == position){
                DialogCrearCategoriaImpl dialog = new DialogCrearCategoriaImpl(DetallesMisTarjetasUno.this);
                dialog.setCancelable(false);
                dialog.show();
            }else{

                TransaccionesObjectDrawer t = (TransaccionesObjectDrawer) tAdapterHolder.getItem(adapterPosition);
                RealmAdministrator.getInstance(getApplicationContext()).updateTransacctionCategory(t.getId(),((tarjetas.dwh.com.tarjetas.model.Categorias)adapter.getItem(position)).getId());

                t.setColorCategoria(position+1);
